...
namespace LaneChanger
{
    public class LaneChangerPathFind : PathFind
    {
        ...
        //The VAST majority of this is the stock code
        //some of the variable names have been replaced for readability
        private void PathFindImplementation(uint unit, ref PathUnit data)
        {
            ...
            //m_position00 seems to hold startPosA if it's available
            LaneChangerPathFind.BufferItem startPosABufferItem;
            if (data.m_position00.m_segment != 0 && num >= 1)
            {
                ...
                startPosABufferItem.m_laneID = this.m_startLaneA;
                startPosABufferItem.m_position = data.m_position00;
                this.GetLaneDirection(data.m_position00, out startPosABufferItem.m_direction, out startPosABufferItem.m_lanesUsed);
                startPosABufferItem.m_comparisonValue = 0f;
            }
            else
            {
                this.m_startLaneA = 0u;
                this.m_startOffsetA = 0;
                startPosABufferItem = default(LaneChangerPathFind.BufferItem);
            }
            //m_position02 seems to hold startPosB, if available
            LaneChangerPathFind.BufferItem startPosBBufferItem;
            if (data.m_position02.m_segment != 0 && num >= 3)
            {
                ...
                startPosBBufferItem.m_laneID = this.m_startLaneB;
                startPosBBufferItem.m_position = data.m_position02;
                this.GetLaneDirection(data.m_position02, out startPosBBufferItem.m_direction, out startPosBBufferItem.m_lanesUsed);
                startPosBBufferItem.m_comparisonValue = 0f;
            }
            else
            {
                this.m_startLaneB = 0u;
                this.m_startOffsetB = 0;
                startPosBBufferItem = default(LaneChangerPathFind.BufferItem);
            }
            
            //m_position01 => endPosA
            LaneChangerPathFind.BufferItem endPosABufferItem;
            if (data.m_position01.m_segment != 0 && num >= 2)
            {
                this.m_endLaneA = PathManager.GetLaneID(data.m_position01);
                endPosABufferItem.m_laneID = this.m_endLaneA;
                endPosABufferItem.m_position = data.m_position01;
                this.GetLaneDirection(data.m_position01, out endPosABufferItem.m_direction, out endPosABufferItem.m_lanesUsed);
                endPosABufferItem.m_methodDistance = 0f;
                endPosABufferItem.m_comparisonValue = 0f;
            }
            else
            {
                this.m_endLaneA = 0u;
                endPosABufferItem = default(LaneChangerPathFind.BufferItem);
            }

            //m_position03 => endPosB
            LaneChangerPathFind.BufferItem endPosBBufferItem4;
            if (data.m_position03.m_segment != 0 && num >= 4)
            {
                this.m_endLaneB = PathManager.GetLaneID(data.m_position03);
                endPosBBufferItem4.m_laneID = this.m_endLaneB;
                endPosBBufferItem4.m_position = data.m_position03;
                this.GetLaneDirection(data.m_position03, out endPosBBufferItem4.m_direction, out endPosBBufferItem4.m_lanesUsed);
                endPosBBufferItem4.m_methodDistance = 0f;
                endPosBBufferItem4.m_comparisonValue = 0f;
            }
            else
            {
                this.m_endLaneB = 0u;
                endPosBBufferItem4 = default(LaneChangerPathFind.BufferItem);
            }

            //m_position11 may hold current position
            ...
            //This looks like a Minimax search (?) maybe
            ...
            if (endPosABufferItem.m_position.m_segment != 0)
            {
                this.m_bufferMax[0]++;
                this.m_buffer[++this.m_bufferMaxPos] = endPosABufferItem;
            }
            if (endPosBBufferItem4.m_position.m_segment != 0)
            {
                this.m_bufferMax[0]++;
                this.m_buffer[++this.m_bufferMaxPos] = endPosBBufferItem4;
            }
            bool flag = false;
            while (this.m_bufferMinPos <= this.m_bufferMaxPos)
            {
                ...
                else
                {
                    ...
                    if (bufferItem6.m_position.m_segment == startPosABufferItem.m_position.m_segment && bufferItem6.m_position.m_lane == startPosABufferItem.m_position.m_lane)
                    {
                    ...
                    if (bufferItem6.m_position.m_segment == startPosBBufferItem.m_position.m_segment && bufferItem6.m_position.m_lane == startPosBBufferItem.m_position.m_lane)
                    {
            ...
            if ((position.m_segment != endPosABufferItem.m_position.m_segment || position.m_lane != endPosABufferItem.m_position.m_lane || position.m_offset != endPosABufferItem.m_position.m_offset) && (position.m_segment != endPosBBufferItem4.m_position.m_segment || position.m_lane != endPosBBufferItem4.m_position.m_lane || position.m_offset != endPosBBufferItem4.m_position.m_offset))
            {
                ...
                if ((byte)(lane2.m_finalDirection & direction2) != 0)
                {
                    if (lane2.CheckType(laneType2, this.m_vehicleTypes) && (segmentID != item.m_position.m_segment || num12 != (int)item.m_position.m_lane) && (byte)(lane2.m_finalDirection & direction2) != 0)
                    {
                        ...
                        if (lane2.m_laneType != NetInfo.LaneType.Pedestrian || item2.m_methodDistance < 1000f)
                        {
                            ...
                            /*
                            if (item.m_position.m_segment == 32086 || item2.m_position.m_segment == 32086 || item.m_position.m_segment == 30867 || item2.m_position.m_segment == 30867 || item.m_position.m_segment == 11329 || item2.m_position.m_segment == 11329)
                            {
                                Debug.Log("XItem1 -> d: " + item.m_direction + " s:" + item.m_position.m_segment + " sn:" + instance.m_segments.m_buffer[item.m_position.m_segment].m_startNode + " en: " + instance.m_segments.m_buffer[item.m_position.m_segment].m_endNode + " l:" + item.m_position.m_lane + "/" + item.m_laneID + " o: " + item.m_position.m_offset + " fl:" + instance.m_lanes.m_buffer[item.m_laneID].m_flags + " lt: " + instance.m_segments.m_buffer[item.m_position.m_segment].Info.m_lanes[item.m_position.m_lane].m_laneType + "\n" +
                                    "XItem2 -> d: " + item2.m_direction + " s:" + item2.m_position.m_segment + " sn:" + instance.m_segments.m_buffer[item2.m_position.m_segment].m_startNode + " en: " + instance.m_segments.m_buffer[item2.m_position.m_segment].m_endNode + " l:" + item2.m_position.m_lane + "/" + item2.m_laneID + " o: " + item2.m_position.m_offset + " fl:" + instance.m_lanes.m_buffer[item2.m_laneID].m_flags + " lt: " + instance.m_segments.m_buffer[item2.m_position.m_segment].Info.m_lanes[item2.m_position.m_lane].m_laneType);
                            }
                            */ 
                            ...
                            //Debug.Log("ITEM:" + item.m_position.m_segment + " ITEM2: " + item2.m_position.m_segment);
        ...
        private void ProcessItem(LaneChangerPathFind.BufferItem item, ushort targetNode, ushort segmentID, ref NetSegment segment, byte connectOffset, int laneIndex, uint lane)
        {
            ...
            if (laneIndex < num)
            {
                ...
                if (lane3.m_laneType != NetInfo.LaneType.Pedestrian || item2.m_methodDistance < 1000f)
                {
                    ...
                    item2.m_laneID = lane;
                    item2.m_lanesUsed = (item.m_lanesUsed | lane3.m_laneType);
                    //Debug.Log("Item1 -> s:" + item.m_position.m_segment + " l:" + item.m_position.m_lane + "/" + item.m_laneID + " o: " + item.m_position.m_offset + " fl:" + instance.m_lanes.m_buffer[item.m_laneID].m_flags + " lt: " + instance.m_segments.m_buffer[item.m_position.m_segment].Info.m_lanes[item.m_position.m_lane].m_laneType + "\n" + 
                    //    "Item2 -> s:" + item2.m_position.m_segment + " l:" + item2.m_position.m_lane + "/" + item2.m_laneID + " o: " + item2.m_position.m_offset + " fl:" + instance.m_lanes.m_buffer[item2.m_laneID].m_flags + " lt: " + instance.m_segments.m_buffer[item2.m_position.m_segment].Info.m_lanes[item2.m_position.m_lane].m_laneType
                    //);
                    this.AddBufferItem(item2, item.m_position);
                }
            }
